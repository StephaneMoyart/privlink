...
export const Calendar: React.FC<CalendarProps> = ({ dateName }) => {
    ...
    return (
        <>
            <div className="w-full border rounded-sm border-black/20 shadow-sm shadow-black/20">{formatedDate}</div>
            ...
            <div className="flex gap-2 flex-col items-center p-2 w-[250px] border border-black/20 rounded-md shadow-black/20 shadow-sm">
                ...
                <div className="w-full grid grid-rows-6 grid-cols-7">
                    ...
                    {[...Array(daysInMonth)].map((_, i) => (
                        <span
                            onClick={() => setDate(new Date(currentYear, currentMonth, i + 1))}
                            className={`text-center cursor-pointer ${checkIfToday(i) ? "text-green-500" : ""} ${checkIfSelected(i) ? "bg-green-300" : ""}`}
                            key={`day-${i}`}
                        >
                            {i + 1}
                        </span>
