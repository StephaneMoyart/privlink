...
export const InputWLabel: React.FC<InputProps> = ({ name, label, type = "text", state }) => {
    ...
    return (
        <div className="relative">
            <input
                id={name}
                className="peer w-full border bg-white border-black/20 rounded-md h-12 px-4 outline-none focus:border-zinc-950 transition-all duration-300 autofill:shadow-[inset_0_0_0px_1000px_rgb(255,255,255)]"
                type={type}
                name={name}
                required
