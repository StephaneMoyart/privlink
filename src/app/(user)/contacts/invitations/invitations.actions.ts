...
export const getContactInvitations = async () => {
    ...
    return await query(`
        SELECT
            p.id AS invited_by_id,
            p.firstname,
            p.lastname,
            p.avatar,
            ci.id AS id
        FROM person p
        JOIN contact_invitation ci
            ON p.id = ci.invited_by_id
...
export const acceptContactInvitationAction = async (invitedByUserId: string, invitationId: string): Promise<void> => {
    ...
    try {
        ...
        await client.query(`
            INSERT INTO contact (person1_id, person2_id)
            VALUES ($1, $2)
        `, [session.id, invitedByUserId])
        ...
        `, [createdConversation.rows[0].id, session.id, invitedByUserId])
        await client.query(`
            DELETE FROM contact_invitation ci
            WHERE ci.id = $1
        `, [invitationId])
